  ...
  const getSourceBadge = (source) => {
    switch (source) {
      case 'shodhganga_import':
        return <span className="bg-amber-light text-amber-800 border border-amber-border text-[10px] font-bold px-2 py-0.5 rounded uppercase">Shodhganga Theses Import</span>;
      case 'scholar_import':
        return <span className="bg-blue-50 text-blue-800 border border-blue-200 text-[10px] font-bold px-2 py-0.5 rounded uppercase">Google Scholar Scan</span>;
      case 'aishe_import':
        return <span className="bg-purple-50 text-purple-800 border border-purple-200 text-[10px] font-bold px-2 py-0.5 rounded uppercase">AISHE Catalog</span>;
      case 'manual_review':
        return (
          <span className="bg-green-light text-green-700 border border-green-border text-[10px] font-bold px-2 py-0.5 rounded uppercase flex items-center space-x-1">
            <CheckCircle2 className="w-2.5 h-2.5 text-green" />
            <span>HEI Verified</span>
          </span>
        );
      default:
        return <span className="bg-slate-100 text-slate-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase">{source}</span>;
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {profile && (
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex items-center space-x-4">
            {/* Completeness Meter */}
            <div className="bg-canvas border border-slate-200 p-3.5 rounded-xl min-w-[170px]">
              <div className="flex justify-between items-center text-xs font-bold text-navy mb-1.5">
            ...
            <button
              ...
              disabled={importing}
              className="px-4 py-2.5 rounded-xl bg-navy hover:bg-navy-light text-white font-bold text-xs transition-colors shadow-sm flex items-center space-x-2 disabled:opacity-50"
            >
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          <div className="py-12 text-center text-slate-500 text-xs">Loading institutional expertise graph...</div>
        ) : profile?.departments?.length === 0 ? (
          <div className="bg-white p-12 rounded-xl border border-slate-200 text-center space-y-3">
            <Layers className="w-8 h-8 text-slate-400 mx-auto" />
            <h3 className="text-sm font-bold text-navy">No departments cataloged yet</h3>
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-100">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 font-semibold text-xs border border-slate-200 flex items-center space-x-1.5 self-start sm:self-auto transition-colors"
                  >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {dept.discipline_tags?.map((tag, idx) => (
                      <span 
                        key={idx}
                        className="bg-teal/10 text-navy font-semibold text-xs px-2.5 py-1 rounded-md border border-teal/20 flex items-center space-x-1"
                      >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {dept.faculty_profiles?.map(fac => (
                      <div 
                        key={fac.id}
                        className="p-3.5 rounded-xl bg-canvas border border-slate-200 space-y-2"
                      >
                        <div className="flex justify-between items-start">
                          <div className="font-bold text-xs text-navy">{fac.name}</div>
                          {fac.verified ? (
      ...
      {editingDept && (
        ...
        >
          <div className="space-y-4 text-xs">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {deptTags.map((tag, idx) => (
                  <span 
                    key={idx}
                    className="bg-slate-100 border border-slate-300 text-xs px-2.5 py-1 rounded-md text-slate-800 flex items-center space-x-1.5"
                  >
                    <span>{tag}</span>
                    <button
                      type="button"
              ...
              <div className="flex space-x-2">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 rounded-xl border border-slate-300 text-xs focus:outline-none focus:ring-2 focus:ring-teal"
                />
                <button
                  type="button"
            ...
            <div className="p-3 rounded-xl bg-green-light border border-green-border text-green-700 text-xs flex items-center space-x-2">
            ...
            <div className="flex justify-end space-x-3 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl bg-teal text-navy font-bold text-xs hover:bg-teal-hover transition-all shadow-sm"
              >
